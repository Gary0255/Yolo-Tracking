...
class ObjectDetection:
    def __init__(self):
        ...
        # 6 bicycle
        # 7 enter car
        self.count = np.zeros(8, dtype = int) # save data for display catagory
        self.prev_count = np.zeros(8, dtype = int) # save last data help for deduction
        self.data_whe= [] # save all out vehicle id
        self.data_en_p= [] # save inside person id
        self.data_en_c= [] # save inside car id
        self.data_ou = [] # save all out person id
        self.data_ou_count = [] # save out person id that has been count
        self.data_car_count = [] #save out car id that has been count
        self.inside_P = [] # save xy coordinate region of inside person
        self.inside_C = [] # save xy coordinate region of inside car
        self.outside = [] # save xy coordinate region of outside
        self.ignore_IN = [] # save xy coordinate region of ignore in
        self.ignore_OUT = [] # save xy coordinate region of ignore out
        self.ignore_IO = [] # save xy coordinate region or ignore in and out
        self.time = None # save current timestamp
        self.time_region = [] # save xy coordinate region of time
        self.date = None # save date
        self.person_conf = 0.8 # set person configuration number
        self.car_conf = 0.85 # set car configuration number
        self.out_conf = 0.85 # set outside configuration number
        self.vid_stride = None # save current video strive
        self.display_ratio = 1 # save video show ratio/resolution
...
#                print(f"p: {self.person_conf}\nc: {self.car_conf}")
    ...
    def draw_boxes(self):
        ...
        cv2.putText(self.plotted_img, result, org=(10,20), fontFace=0,
                     fontScale=0.5*self.display_ratio, thickness = 2*self.display_ratio, color=(255, 255, 255))
        if self.ignore_IO:
            for j in (self.ignore_IO):
                cv2.rectangle(self.plotted_img, j[0], j[1], color=(165,42,42), thickness=2, lineType=cv2.LINE_AA)
        if self.ignore_IN:
            for j in (self.ignore_IN):
                cv2.rectangle(self.plotted_img, j[0], j[1], color=(65, 105, 225), thickness=2, lineType=cv2.LINE_AA)
        if self.inside_C:
            for j in (self.inside_C):
                cv2.rectangle(self.plotted_img, j[0], j[1], color=(95,158,160), thickness=2, lineType=cv2.LINE_AA)
        if self.ignore_OUT:
            for j in (self.ignore_OUT):
                cv2.rectangle(self.plotted_img, j[0], j[1], color=(51, 68, 255), thickness=2, lineType=cv2.LINE_AA)
        if self.inside_P:
            for j in (self.inside_P):
                cv2.rectangle(self.plotted_img, j[0], j[1], color=(0, 255, 0), thickness=2, lineType=cv2.LINE_AA)
        if self.outside:
            for j in (self.outside):
                cv2.rectangle(self.plotted_img, j[0], j[1], color=(0, 0, 255), thickness=2, lineType=cv2.LINE_AA)
...
class BasePredictor:
    ...
    def setup_source(self, source):
        ...
        self.dataset = load_inference_source(source=source, imgsz=self.imgsz, vid_stride=self.vid_stride)
...
#        w, h = im0.shape[1], im0.shape[0]
        self.draw_boxes()
        (w,h) = (int(self.dataset.w/self.display_ratio), int(self.dataset.h/self.display_ratio))
    ...
    def write_results(self, idx, results, batch):
        ...
        if result.boxes.id is not None:
            ...
            for i, j in enumerate(boxes):
                if self.check_conf(cls[i], conf[i]) or self.dataset.error:
...
#            print(f"VALID ID: {valid_id}\nVALID CLASS: {valid_cls}\n--------------------------")
            self.count_obj(centre, valid_cls, valid_id, valid_conf, self.dataset.error)
